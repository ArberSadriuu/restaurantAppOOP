...
class RestaurantAppGUI(MDApp):
    ...
    def build(self):
        ...
        # shtoni lidhje të re për shtypjen e rreshtave                                                       
        menu_table.bind(on_row_press=self.on_row_press)
        # shtimi i tabeles se të dhënave të menusë në array-n e ekraneve të fëmijëve
        screen.children[0].add_widget(menu_table)
        return screen
    # modifiko metodën për të vendosur të dhënat e rreshtit të zgjedhur    
    def checked(self, instance_table, current_row):
        print(current_row)
        
    # shtimi i metodes së re për zgjedhjet e rreshtave
    def on_row_press(self, instance_table, instance_row):
        # merrni numrin 'real' të rreshtit duke e ndarë indeksin e rreshtit me të dhënat e kolonës së tabelës së shembullit
        # Këshillë: instance_row.index numërohet gjithmonë bazuar në të dhënat e kolonës
